...
import { VoucherResponse, VoucherType, useGetAllVouchers, useGetProfile } from '@queries';
    ...
    <Stack justifyContent="space-between" gap={2}>
      ...
      <RadioGroup onChange={handleChangeVoucher} value={voucherId}>
        <Stack width="100%" gap={2}>
          {vouchers?.filter((voucher) => !voucher.metadata.users.includes(profile?.id))?.length >
          0 ? (
            vouchers
              ?.filter((voucher) => !voucher.metadata.users.includes(profile?.id))
              ?.map((voucher) => {
                return (
                  <Card
                    key={voucher.id}
